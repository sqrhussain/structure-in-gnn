...
class FeatureOnlyData(InMemoryDataset):
    ...
    def download(self):
        copyfile('{}/{}/{}'.format(self.featsUrl, self.name, self.raw_file_names[0]),'{}/{}'.format(self.raw_dir, self.raw_file_names[0]))

    def process(self):
        features_path = osp.join(self.raw_dir,self.raw_file_names[0])
        data = load_embedding(embFile = None,featFile = features_path)
