...
class RepeatedConfigurationModelCitationNetwork(CitationNetwork):
    
    conf_model_url = '../data/processed/repeated-conf_model'
    def __init__(self, root, name, init_num, directed = True,reverse = False): #, transform=None, pre_transform=None):
        self.init_num = init_num
        super(RepeatedConfigurationModelCitationNetwork, self).__init__(root, name, directed,reverse)
        self.directed = directed
        self.reverse = reverse
    ...
    def download(self):
        copyfile('{}/{}/{}'.format(self.url, self.name, self.raw_file_names[1]),
            '{}/{}'.format(self.raw_dir, self.raw_file_names[1]))
        copyfile('{}/{}/{}/{}'.format(self.conf_model_url, self.name, self.init_num, self.raw_file_names[0]),
            '{}/{}'.format(self.raw_dir, self.raw_file_names[0]))
