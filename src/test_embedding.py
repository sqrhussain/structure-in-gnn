...
def report_test_acc_unsupervised_embedding(cache_prefix, dataset, filename, method, speedup=False):
    ...
    for init in range(inits):
        ...
        emb = EmbeddingData(f'data/tmp/{args.method}{nerdString}_{dataset}_{directionality}_{cache_prefix}EmbInit{init}',
                            dataset, filename, directed=isDirected, reverse=isReversed,
                            initialization=f'{method}/{init}',
                            nerd=nerdStatus)
        print(f'started test {init}')
        test = test_method(emb, num_splits=num_splits, speedup=speedup)
        print(str(test) + '\n')
        tests = tests + test
    return tests
...
test_out = f'reports/results/test_acc/emb_eval_{args.method}{nerdString}_{args.dataset}_{directionality}.csv'
