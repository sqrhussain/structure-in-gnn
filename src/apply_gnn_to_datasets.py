from evaluation.gnn_hyperparam_search import eval_archs
...
from data.cora_loader import CitationNetwork, RepeatedConfigurationModelCitationNetwork
import warnings
import pandas as pd
import os
import argparse
...
parser.add_argument('--conf_inits',
				type = int,
				default = 20,
				help = 'Number of configuration model runs. Default is 20.')
...
dataset = CitationNetwork(f'/tmp/{args.dataset}4{("_"+args.directionality) if isDirected else ""}',args.dataset,directed=isDirected,reverse=isReversed)
...
def eval_conf_model(df_val):
    ...
    for i in range(args.conf_inits):
        dataset = RepeatedConfigurationModelCitationNetwork(f'/tmp/{args.dataset}-conf{i}',args.dataset,init_num=i,directed=isDirected,reverse=isReversed)
